...
import { GetIndicatorParams, GetIndicatorForKlinesParams } from "@server/services/indicator.service"
import { getKlines } from "@server/services/kline.service"
const indicators = require('@server/services/indicator.service')

export const getIndicatorForKlines = async (params:GetIndicatorForKlinesParams)=>{
    const klines = await getKlines(params)
    const _priceType:PriceType = params.priceType?params.priceType:PriceType.close
    ...
    if(indicatorSelector in indicators){
        const indicator:(params:GetIndicatorParams)=>Promise<Kline[]> = indicators[indicatorSelector]
